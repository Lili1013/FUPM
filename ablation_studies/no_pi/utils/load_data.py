...
class Load_Data(object):
    ...
    def calculate_inter_num(self,df):
        user_inter_num = {}
        # df['user_inter_num'] = 0
        for x, value in df.groupby('userID'):
            user_inter_num[int(x)] = len(value)
            # df.loc[df['userID']==int(x),'user_inter_num'] = len(value)
        return user_inter_num


    def load_orig_dataset(self):
        self.df_rating = pd.read_csv(self.rating_file_path)[['userID','itemID','rating']]
        self.user_inter_num_dict = self.calculate_inter_num(self.df_rating)
    ...
    def calculate_user_neighbors(self):
        self.user_neighbors_dict = {}
        self.item_inter_dict = self.df_rating.groupby('itemID')['userID'].apply(list).to_dict()
        self.user_inter_dict = self.df_rating.groupby('userID')['itemID'].apply(list).to_dict()
        for key,value in self.user_inter_dict.items():
            user_neighbors = []
            for each_item in value:
                # item_users = self.item_inter_dict[each_item].remove(key)
                item_users = self.item_inter_dict[each_item]
                user_neighbors.extend(item_users)
            user_neighbors = list(set(user_neighbors))
            self.user_neighbors_dict[key] = user_neighbors

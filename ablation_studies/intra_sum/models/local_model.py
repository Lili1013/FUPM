...
class Local_Model(nn.Module):
    ...
    def get_potential_item_emb_orig(self, nodes_u, nodes_v):
        ...
        if masks.any():
            ...
            pos_item_embs = torch.stack(
                [torch.mean((self.v_emb(torch.tensor(items, device=self.device))+self.v_review_feat_emb(torch.tensor(items, device=self.device))),dim=0) for items in pos_item_lists])
        ...
        if (~masks).any():
            ...
            non_inter_item_embs = torch.stack(
                [torch.mean((self.v_emb(torch.tensor(items, device=self.device))+self.v_review_feat_emb(torch.tensor(items, device=self.device))),dim=0) for items in
                 non_inter_items_list])
    ...
    def forward(self, nodes_u, nodes_v,global_protos,inter_nums):
        ...
        u_id_feats = u_id_feats+u_review_feats
        v_id_feats = v_id_feats+v_review_feats

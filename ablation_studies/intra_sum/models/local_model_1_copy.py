...
class Local_Model(nn.Module):
    ...
    def get_potential_item_emb_old(self,nodes_u,nodes_v):
        ...
        for user_id,item_id in zip(nodes_u.tolist(),nodes_v.tolist()):
            ...
            pos_item_emb_list.append(pos_item_emb)
        return torch.stack(pos_item_emb_list)
    def get_potential_item_emb_best(self, nodes_u, nodes_v):
        neg_num = 4
        ...
        if masks.any():
            ...
            pos_item_lists = [random.sample(self.potential_pos_dict[(user_ids[idx].item(), item_ids[idx].item())], neg_num) if len(self.potential_pos_dict[(user_ids[idx].item(), item_ids[idx].item())]) >= neg_num
                              else self.potential_pos_dict[(user_ids[idx].item(), item_ids[idx].item())] if len(self.potential_pos_dict[(user_ids[idx].item(), item_ids[idx].item())]) > 0
        ...
        if (~masks).any():
            ...
            non_inter_items_list = [
                random.sample(list(self.all_items - set(self.local_data.user_inter_dict[user_ids[idx].item()])), neg_num)
    ...
    def get_potential_item_emb(self, nodes_u, nodes_v):
        ...
        if masks.any():
            ...
            random.sample(self.local_data.user_inter_dict[user_ids[idx].item()],neg_num-len(self.potential_pos_dict[(user_ids[idx].item(), item_ids[idx].item())]))
             for idx in indices.tolist()]
            pos_item_embs = torch.stack(
                [torch.mean(self.v_emb(torch.tensor(items, device=self.device)), dim=0) for items in pos_item_lists])
            pos_item_emb_list[indices] = pos_item_embs
        ...
        if (~masks).any():
            ...
            pos_item_emb_list[indices] = non_inter_item_embs
        return pos_item_emb_list
    def get_potential_item_emb_2(self, nodes_u, nodes_v):
        neg_num = 4
        user_ids = nodes_u.to(self.device)
        item_ids = nodes_v.to(self.device)
    ...
    def get_potential_item_emb_1(self, nodes_u, nodes_v):
        neg_num = 4
        user_ids = nodes_u.tolist()
        item_ids = nodes_v.tolist()

        sim_item_list = [random.sample(set(self.local_data.sim_item_dict[item_id])-set(self.local_data.user_inter_dict[user_id]),neg_num)
                         if len(set(self.local_data.sim_item_dict[item_id])-set(self.local_data.user_inter_dict[user_id]))>=neg_num
                         else list(set(self.local_data.sim_item_dict[item_id])-set(self.local_data.user_inter_dict[user_id])) if len(set(self.local_data.sim_item_dict[item_id])-set(self.local_data.user_inter_dict[user_id]))>0
                         else [item_id]
                         for user_id,item_id in zip(user_ids,item_ids)]
        pos_item_embs = torch.stack(
            [torch.mean(self.v_emb(torch.tensor(items, device=self.device)), dim=0) for items in sim_item_list])
        # pos_item_emb_list = pos_item_embs

        return pos_item_embs
    ...
    def forward(self, nodes_u, nodes_v,global_protos,inter_nums):
        ...
        return u_feats, pred_prob.squeeze(), u_id_feats, v_id_feats, u_review_feats, v_review_feats,v_review_feats

...
def generate_test_negatives(index_range, df_all, df_test, df_train, test_neg_num, num_items,dislike_dict):
    negatives = []
    num_negatives = test_neg_num
    i = index_range[0]
    while i < index_range[1]:
        ...
        each_negatives.append(int(uid))
        each_negatives.append(int(iid))
        dislike_items = dislike_dict[int(uid)]
        if len(dislike_items) >= num_negatives:
            j = random.sample(dislike_items,num_negatives)
            each_negatives.extend(j)
        elif len(dislike_items) > 0:
            each_negatives.extend(dislike_items)
            num_negatives_1 = num_negatives-len(dislike_items)
            for t in range(num_negatives_1):
                j = np.random.randint(num_items)
                while (len(df_all[(df_all['userID'].isin([uid])) & (df_all['itemID'].isin([j]))]) > 0) \
                        or (len(df_train[(df_train['userID'].isin([uid])) & (df_train['itemID'].isin([j]))]) > 0):
                    j = np.random.randint(num_items)
...
def generate_and_store_test_negatives(df_all, df_test, df_train, test_neg_num, test_pos_neg_path,dislike_dict):
    ...
    func = partial(generate_test_negatives, df_all=df_all, df_test=df_test, df_train=df_train,
                   test_neg_num=test_neg_num, num_items=num_items,dislike_dict=dislike_dict)
    ...
    if index_ranges[-1][1] < len(df_test):
        ...
        negatives = generate_test_negatives(index_ranges, df_all, df_test, df_train, test_neg_num, num_items,dislike_dict)
...
if __name__ == '__main__':
    ...
    domain = 'sport'
    # dataset = 'douban_book_movie_music'
    # domain = 'music'
    ...
    to_path = f'../../../datasets/{dataset}/{domain}/test_0.3.txt'
    with open(f'../../../datasets/{dataset}/{domain}/user_dislike_items_0.3.pkl', 'rb') as f:
    ...
    print(len(test_data))
    generate_and_store_test_negatives(df, test_data, train_data, 99,to_path,dislike_dict)

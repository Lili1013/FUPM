...
def calculate_hr_ndcg(model, test_ratings, test_negatives, k,device,overlap_user_protos,user_inter_nums):
    global _model
    ...
    global _overlap_user_protos
    global _user_inter_nums
    _model = model
    _test_ratings = test_ratings
    _test_negatives = test_negatives
    _k = k
    _device = device
    _overlap_user_protos = overlap_user_protos
    _user_inter_nums = user_inter_nums
    ...
    for idx in range(len(_test_ratings)):
        ...
        r = ranklist_by_heapq(items, pred.detach().cpu(), 10)
        hr, ndcg, precision, recall, mrr = eval_one_user(r, test_pos_items)
        if hr == 0:
            failure_users.append(u_id)
            failure_items.append(test_pos_items[0])

    df_f = pd.DataFrame({
        'user_id':failure_users,
        'item_id':failure_items
    })
    df_f.to_csv('failure_case.csv',index=False)
    ...
    hr, ndcg, mrr = obtain_final_result(result,test_users)
    ...
    hr_5, ndcg_5, mrr_5 = obtain_final_result(result_5,test_users)
    pool.close()

    return hr, ndcg, mrr,hr_5,ndcg_5,mrr_5


def obtain_final_result(source_result,test_users):
...
def pred_one_user(u_id, items):
    ...
    u_feats, pred_prob, u_id_feats,v_id_feats,u_review_feats,v_review_feats,_= _model.forward(user_id_input, item_id_input,_overlap_user_protos,_user_inter_nums)
    return pred_prob

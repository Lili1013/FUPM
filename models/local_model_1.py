...
class Local_Model(nn.Module):
    def __init__(self, **params):
        ...
        self.dislike_item_dict = params['dislike_path']
    ...
    def get_potential_item_emb_1(self, nodes_u, nodes_v):
        ...
        if (~masks).any():
            ...
            non_inter_items_list = [
                random.sample(self.dislike_item_dict[user_ids[idx].item()], neg_num)
                if len(self.dislike_item_dict[user_ids[idx].item()]) >= neg_num
                else self.dislike_item_dict[user_ids[idx].item()] if len(self.dislike_item_dict[user_ids[idx].item()]) > 0
                else [item_ids[idx].item()]
                for idx in indices.tolist()]
    ...
    def forward(self, nodes_u, nodes_v,global_protos,inter_nums):
        ...
        potential_item_id_feats = self.get_potential_item_emb_1(nodes_u,nodes_v)
        ...
        elif self.args.interpo_way == 'fixed':
